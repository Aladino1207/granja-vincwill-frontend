  ...
  try {
    ...
    registros.forEach(reg => {
      ...
      tr.innerHTML = `
        ...
        <td>${reg.cantidad.toFixed(2)}</td> <!-- Mostramos la cantidad base con 2 decimales -->
...
async function guardarSalud(e) {
  ...
  if (!tipoEventoEl) {
      alert("Error crítico: No se encuentra el campo 'tipoEvento' en el HTML.");
      return;
  }
  const tipo = tipoEventoEl.value;
...
function abrirFormularioSalud() {
    ...
    if(formContainer) formContainer.classList.add('is-open');
}
function cerrarFormularioSalud() {
    const formContainer = document.getElementById('formContainer');
    if(formContainer) formContainer.classList.remove('is-open');
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (tipoEvento) {
      tipoEvento.addEventListener('change', () => {
          ...
          const tratamientoGroup = document.getElementById('tratamientoGroup');
          const mortalidadGroup = document.getElementById('mortalidadGroup');
          const diasRetiroGroup = document.getElementById('diasRetiroGroup');

          // Seguridad por si no existen los divs en el HTML
          if(vacunaGroup) vacunaGroup.style.display = 'none';
          if(tratamientoGroup) tratamientoGroup.style.display = 'none';
          if(mortalidadGroup) mortalidadGroup.style.display = 'none';
          if(diasRetiroGroup) diasRetiroGroup.style.display = 'block'; // Default visible

          if (val === 'Vacunacion' && vacunaGroup) vacunaGroup.style.display = 'block';
          else if (val === 'Tratamiento' && tratamientoGroup) tratamientoGroup.style.display = 'block';
          else if (val === 'Mortalidad' && mortalidadGroup) {
              mortalidadGroup.style.display = 'block';
              if(diasRetiroGroup) diasRetiroGroup.style.display = 'none';
          }
      });
  } else {
      console.warn("Elemento 'tipoEvento' no encontrado en el DOM. Revisa tu salud.html");
  }
  ...
  if (toggleBtn) {
      toggleBtn.onclick = () => {
          ...
          // CORRECCIÓN: Verificar si tipoEvento existe antes de usarlo
          if (tipoEvento) {
             tipoEvento.dispatchEvent(new Event('change'));
          } else {
             // Fallback si no existe: aseguramos que se vea algo por defecto
             console.log("Abriendo formulario sin reset de tipo (elemento no encontrado)");
          }
